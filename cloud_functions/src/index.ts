...
// === Translation System =======================================================
// Cloud Functions run server-side and have no access to the Flutter l10n system.
// Instead, we store the user's locale preference ('en', 'pl', etc.) in their
// Firestore document under the `locale` field. This is synced from the app
// whenever the user changes their language via LocaleProvider._syncLocaleToFirestore().
//
// To add a new language: add a new key to MESSAGES matching the locale code,
// and translate all MessageKey entries.
//
// To add a new message: add the key to the MessageKey union type,
// then add the translation to every language block in MESSAGES.
...
type MessageKey =
  ...
  | 'order_accepted_body'
  | 'order_accepted_pickup_body'
  | 'rider_on_way_title'
  | 'rider_on_way_body'
  | 'food_ready_title'
  | 'food_ready_body'
  | 'ready_for_pickup_title'
  | 'ready_for_pickup_body'
  | 'delivered_title'
  | 'delivered_body'
  | 'pickup_collected_title'
  | 'pickup_collected_body'
  | 'new_order_title'
  | 'new_order_body'
  | 'order_ready_rider_title'
  | 'order_ready_rider_body'
  | 'new_delivery_title'
  | 'new_delivery_body'
  | 'admin_notif_default_title'
  | 'admin_notif_default_body';
...
async function createOrderAndDispatch({
  ...
  await notifyUser(
    ...
    t('en', 'new_order_title'),
    t('en', 'new_order_body', { id: orderID.slice(0, 8) }),
    "order",
    "users",
  ...
  await notifyUser(
    userID,
    user.fcmToken ?? null,
    t(locale, 'order_placed_title'),
    t(locale, 'order_placed_body', { restaurant: restaurant.name ?? '' }),
    "order",
    "users",
...
async function dispatchToRider(
  ...
  const riderLocale = (rider.locale as string) ?? 'en';
  ...
  await notifyUser(
    riderDoc.id,
    rider.fcmToken ?? null,
    t(riderLocale, 'new_delivery_title'),
    t(riderLocale, 'new_delivery_body', {
...
export const onDispatchJobAccepted = fsEvents.onDocumentUpdated(
  ...
  async (event) => {
    ...
    await notifyUser(
      userID,
      user.fcmToken ?? null,
      t(locale, 'rider_on_way_title'),
      t(locale, 'rider_on_way_body'),
      "order",
      "users",
...
export const onOrderStatusChanged = fsEvents.onDocumentUpdated(
  ...
  async (event) => {
    ...
    const locale   = (customer.locale as string) ?? 'en';
    ...
    if (status === "In Progress") {
      await notifyUser(
        ...
        t(locale, 'order_accepted_title'),
        isPickup
          ? t(locale, 'order_accepted_pickup_body')
          : t(locale, 'order_accepted_body'),
        "order",
        "users",
    ...
    if (status === "Ready") {
      if (isPickup) {
        ...
        await notifyUser(
          userID,
          customer.fcmToken ?? null,
          t(locale, 'ready_for_pickup_title'),
          t(locale, 'ready_for_pickup_body'),
          "order",
          "users",
          { type: "ORDER_STATUS", orderID, status }
        );
        ...
        await notifyUser(
          userID,
          customer.fcmToken ?? null,
          t(locale, 'food_ready_title'),
          t(locale, 'food_ready_body'),
          "order",
          "users",
          { type: "ORDER_STATUS", orderID, status }
        );
        ...
        if (riderUID && riderUID.length > 5) {
          ...
          await notifyUser(
            riderUID,
            rider?.fcmToken ?? null,
            t(riderLocale, 'order_ready_rider_title'),
            t(riderLocale, 'order_ready_rider_body', {
    ...
    if (status === "Delivered") {
      await notifyUser(
        userID,
        customer.fcmToken ?? null,
        isPickup ? t(locale, 'pickup_collected_title') : t(locale, 'delivered_title'),
        isPickup ? t(locale, 'pickup_collected_body') : t(locale, 'delivered_body'),
        "order",
        "users",
        { type: "ORDER_STATUS", orderID, status }
      );
