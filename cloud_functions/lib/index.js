...
async function createOrderAndDispatch({ quoteID, userID, restaurantID, paymentMethod, paymentDetails, }) {
    ...
    await notifyUser(restaurantID, restaurant.fcmToken ?? null, `${EMOJI.BELL} New Order!`, `Order #${orderID.slice(0, 8)} received`, "order", "users", { type: "NEW_ORDER", orderID });
    await notifyUser(userID, user.fcmToken ?? null, `Order Placed! ${EMOJI.SUCCESS}`, `Your order from ${restaurant.name ?? "the restaurant"} has been received.`, "order", "users", { type: "ORDER_STATUS", orderID, status: "Pending" });
...
async function dispatchToRider(orderID, order, restaurant, customer) {
    ...
    await notifyUser(riderDoc.id, rider.fcmToken ?? null, `${EMOJI.SCOOTER} New Delivery!`, `${restaurant.name ?? "Restaurant"} · zł${riderEarnings} · ${paymentLabel}`, "order", "riders", { type: "DISPATCH_JOB", orderID });
}
/* ---------------------------------------------- */
...
exports.onDispatchJobAccepted = fsEvents.onDocumentUpdated({ document: "dispatch_jobs/{jobId}", region: REGION }, async (event) => {
    ...
    const user = userDoc.data();
    await notifyUser(userID, user.fcmToken ?? null, `Rider on the way! ${EMOJI.SCOOTER}`, "Your rider is heading to the restaurant.", "order", "users", { type: "ORDER_STATUS", orderID, status: "In Progress" });
});
exports.onOrderStatusChanged = fsEvents.onDocumentUpdated({ document: "orders/{orderID}", region: REGION }, async (event) => {
    ...
    if (status === "In Progress") {
        await notifyUser(userID, customer.fcmToken ?? null, `Order Accepted ${EMOJI.CHECK}`, isPickup
            ? "The restaurant is preparing your order. We'll notify you when it's ready."
            : "The restaurant is preparing your order.", "order", "users", { type: "ORDER_STATUS", orderID, status });
    }
    if (status === "Ready") {
        if (isPickup) {
            await notifyUser(userID, customer.fcmToken ?? null, `Ready for Collection! ${EMOJI.STORE}`, "Your order is ready. See you soon!", "order", "users", { type: "ORDER_STATUS", orderID, status });
        }
        else {
            // For Delivery: Tell customer the kitchen is done
            await notifyUser(userID, customer.fcmToken ?? null, `Food is Ready!`, "The restaurant has finished your order. Your rider will pick it up shortly.", "order", "users", { type: "ORDER_STATUS", orderID, status });
            ...
            if (riderUID && riderUID.length > 5) {
                ...
                const rider = riderDoc.data();
                await notifyUser(riderUID, rider?.fcmToken ?? null, `Order Ready for Pickup ${EMOJI.STORE}`, `${after.restaurantName ?? "The restaurant"} has the order ready.`, "order", "riders", { type: "ORDER_READY", orderID });
            }
            else {
    ...
    if (status === "Delivered") {
        await notifyUser(userID, customer.fcmToken ?? null, isPickup ? `Enjoy! ${EMOJI.SUCCESS}` : `Delivered! ${EMOJI.SUCCESS}`, isPickup ? "Order collected. Enjoy your meal!" : "Your order has arrived!", "order", "users", { type: "ORDER_STATUS", orderID, status });
        // Free up the rider
