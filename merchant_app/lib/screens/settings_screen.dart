...
import 'package:merchant_app/widgets/map_dialog.dart';
...
class _BusinessInfoCardState extends State<_BusinessInfoCard> {
  ...
  void initState() {
    ...
    try {
      _mobileController = PhoneController(
        initialValue: PhoneNumber.parse(_initials["mobile"]),
      );
    } catch (_) {
      _mobileController = PhoneController(
  ...
  void _setInitials() {
    _initials = {
      ...
      "iban": widget.data["iban"] ?? '',
      "mobile": widget.data["businessMobile"] ?? '',
      "address": widget.data["address"] ?? '',
      "lat": (widget.data["lat"] as num?)?.toDouble(),
      "lng": (widget.data["lng"] as num?)?.toDouble(),
    };
  }
  ...
  void didUpdateWidget(_BusinessInfoCard oldWidget) {
    ...
    if (widget.data != oldWidget.data) {
      ...
      if (!_edited) {
        ...
        _lng = _initials["lng"];
        try {
          _mobileController.value = PhoneNumber.parse(_initials["mobile"]);
        } catch (_) {}
      }
    }
  }
...
class _UserProfileCardState extends State<_UserProfileCard> {
  ...
  void _onEdit() {
    final currentPhone = _phoneController.value.international;
